 ...
 * at the next level, we have "leetcode.com", and at the lowest level, "discuss.leetcode.com". When we visit a domain like "discuss.leetcode.com",
...
public class SubdomainVisitCount {
    /**
     * Approach: Maintain a hashmap containing count of all the domains visited so far.
     * For each domain, extract out all the top level domains and increment their frequency.
     */
    public List<String> subdomainVisits(String[] cpdomains) {
        ...
        for (String cpdomain : cpdomains) {
            ...
            for (int i = 0; i < domain.length(); i++) {
                ...
                if (c == '.') { //remaining string would be a top level domain
                    String suffix = domain.substring(i + 1);
                    map.put(suffix, map.getOrDefault(suffix, 0) + freq);
                }
            }
        }
