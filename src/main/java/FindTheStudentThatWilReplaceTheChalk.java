 ...
 * </pre>
 */
public class FindTheStudentThatWilReplaceTheChalk {
    /**
     * Approach: Modular arithmetic, find the total sum and then perform modulo on the total chalks available.
     * <p>
     * {@link SubarraySumsDivisibleByK} {@link MakeSumDivisibleByP} {@link SentenceScreenFitting}
     */
    public int chalkReplacer(int[] chalk, int k) {
        long sum = 0; //gotcha is to use a long instead of int to avoid integer overflow. Got a WA in the contest because of this silly mistake
        for (int c : chalk) {
            sum += c;
        }
        k %= sum; //make the k wrap around
        long curChalks = 0;
        for (int i = 0; i < chalk.length; i++) {
            curChalks += chalk[i];
            if (curChalks > k) { //if chalks required exceeds available chalks, ith student needs to replace the chalk
                return i;
            }
        }
        return -1;
